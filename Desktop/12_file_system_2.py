...
# 폴더 생성 : 이미 폴더가 있는데 생성하면 에러가 남
# 폴더를 만드는 거라 위에 .txt 같은 확장자 붙여도 폴더 생성함
#os.mkdir(folder_path + "new_folder.txt")
...
#shutil.rmtree(folder_path + "new_folder") # 폴더에 내용이 있어도 삭제 가능
# 모든 파일이 삭제될 수 있으니 주의 (휴지통에도 안가고 ctrl + z 도 안먹음 그냥 완전삭제)


# 파일 복사하기
#shutil.copy("Excel/File/Sample_Formula.xlsx", "Desktop/File") # 복사할 원본 파일 경로, 붙여넣을 경로 
# 다른 이름으로 파일 복사하기
...
# 폴더 이동
#shutil.move("Desktop/File/copy.xlsx", "Desktop/File/File2")
shutil.move("Desktop/File/copy.xlsx", "Desktop/File/File_Move_Renamadsadsad.txt") # 폴더를 못찾으면 이름을 바꿈
